...
auto compute_similarity_matrix( std::map<uint32_t, std::map<uint32_t,uint32_t>> & movies,
				std::map<uint32_t,uint32_t> & similarity_matrix ) -> void {
  //std::cout << "Movie count is : " << movies.size() << std::endl;
...
auto main( int argc, char **argv ) -> int {
  ...
  std::map<uint32_t,std::map<uint32_t,uint32_t>> movies;
  std::map<uint32_t, std::map<uint32_t,uint32_t>> users;
  std::map<uint32_t,uint32_t> similarity_matrix;
  
  read_movie_file( s, movies, users );
  //  compute_statistics( movies, users );
  compute_similarity_matrix( movies, similarity_matrix );
