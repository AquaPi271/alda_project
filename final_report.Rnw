...
-- The first experiment will determine which k-NN is the best setting for each combination of input.  Results will be computed against the M1500\_U500 subsample with further subsamples depending on results.  The $k$ parameter will vary from 1 to 20 (or more) to find a maximized setting.  The results of these experiments will then set the k-value for all subsequent tests.
