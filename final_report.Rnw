...
In the density table, the results reveal that for densities down to 20\% the top collaborative filtering methods continue to signicantly outperform the baseline metric.  While the appears to be a general trend of worsening results towards the lower densities, the baseline also tracks in this direction.  This indicates that the crossover point is much lower than 20\%, possible in low single digit density regions.  This is a testament to the success of these algorithms, especially where data is well represented.
...
These plots show converging results after movie and user count exceed a certain portion of the subsamples.  The trend would indicate that the IBCF performs better than UBCF for the dataset subsamples.  There is a wide variability at low user count settings for both CF metrics suggesting not enough user information is available when the subsample is low compared to the overall dataset.  This indicates that future experiments should possibly consider some threshold percentage of both data dimensions before results start to stabilize.  Where the results start to converge IBCF appears to outperform UBCF, marginally so.  In this dataset the number of movies is outstripped by the total user count, indicating more useful information is stored in the item-item matrix.

\subsection{Miscellaneous Results}

In cases where data points were missing so as not to match the knn threshold, the average rating was used instead.  This proved to have insignificant effect on the data.
